...
impl DirOps for RootDirOps {
    fn lookup_child(
        ...
        if let Ok(pid) = name.parse::<RawPid>() {
            // 检查进程是否存在
            if let Some(process_ref) = crate::process::ProcessManager::find_task_by_vpid(pid) {
                ...
                // 创建新的 PID 目录
                let inode = PidDirOps::new_inode(process_ref, dir.self_ref_weak().clone());
    ...
    fn populate_children<'a>(
        ...
        let process_list = {
            ...
            if let Some(process_map) = all_processes.as_ref() {
                process_map
                    .iter()
                    .map(|(pid, process_ref)| (*pid, process_ref.clone()))
                    .collect::<Vec<_>>()
            } else {
                Vec::new()
            }
        ...
        // 填充进程目录
        for (pid, process_ref) in process_list {
            cached_children
                .entry(pid.to_string())
                .or_insert_with(|| PidDirOps::new_inode(process_ref, dir.self_ref_weak().clone()));
        }
